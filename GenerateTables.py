...
os.environ['ICTDIR']='/n/holystore01/LABS/guenette_lab/Lab/data/NEXT/FLEX/mc/eres_22072022/IC'
sys.path.insert(1, '/n/holystore01/LABS/guenette_lab/Lab/data/NEXT/FLEX/mc/eres_22072022/IC')
...
if __name__ == "__main__":
    ...
    for sqlfile in sql_files:
        print(sqlfile)
        if "SipmNoisePDF" in sqlfile:
            fd = open(sqlfile, "r")
            command = ""  # Initialization of command.
            for line in fd:
                command += line
                if command.find(";") != -1:  # ; has been found. Command should be executed.
                    command = command[0:command.find(";")+1]
                    cursor_sqlite.execute(command)
                    command = ""   # Reinitialization of command.
            print('-> done!')
            conn_sqlite.commit()
            fd.close()
        else:
